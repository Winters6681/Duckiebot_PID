...
"""
    class PIDController: implement the PID controller
    def __init__(self, kp, ki, kd): initialize the PID controller
        initialize self.kp, self.ki, slef.kd with given kp, ki, kd
    def compute(self, setpoint, process_variable, dt): compute the PID output
    ---------------------------------------------------------------------------------------------
    class DuckiebotODEModel: implement the ODE model of the Duckiebot
    def __init__(self, wheel_distance, wheel_radius, omega_max): initialize the Duckiebot model
        initialize self.wheel_distance, self.wheel_radius, self.omega_max with given values
    def dynamics(self, state, v, omega): implement the dynamics of the Duckiebot
    ---------------------------------------------------------------------------------------------
...
def simulate_duckiebot_with_pid(model, pid, setpoint, dt, total_time):
    ...
    #     return model.dynamics(state, v_l, v_r)
    def ode_function(t, state):
        _, _, theta = state
        control_output = pid.compute(setpoint, theta, dt)
...
class LinearizedDuckiebotModel:
    ...
    #         [0, 0],
    #         [-self.wheel_radius/self.wheel_distance, self.wheel_radius/self.wheel_distance]
    #     ])
    #     # B = np.array([
    #     #     [self.wheel_radius/2 * np.cos(theta_operating), self.wheel_radius/2 * np.cos(theta_operating)],
    #     #     [self.wheel_radius/2 * theta_operating, self.wheel_radius/2 * theta_operating],
    #     #     [-self.wheel_radius/self.wheel_distance, self.wheel_radius/self.wheel_distance]
    #     # ])
    ...
    # def linear_dynamics(self, state, v_l, v_r, theta_operating=0.0):
    #     """
...
def simulate_linearized_duckiebot(model, pid, setpoint, dt, total_time):
    ...
    def ode_function(t, state):
        ...
        return model.linear_dynamics(state, 0.5, control_output)
